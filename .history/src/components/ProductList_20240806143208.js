...
const ProductList = () => {
    ...
    const [hoveredProduct, setHoveredProduct] = useState(null); // State để lưu sản phẩm đang hover
                            ...
                            {loadingDogs ? (
                                ...
                                dogs.map((pet) => (
                                    <div
                                        key={pet.id}
                                        className="product"
                                        onMouseEnter={() => setHoveredProduct(pet)}
                                        onMouseLeave={() => setHoveredProduct(null)}
                                        onClick={() => handleProductClick(pet.id)}
                                    >
                                        <img src={"http://localhost:8080/image/" + pet.image} alt={pet.name} />
                                        <p>{pet.name}</p>
                                        {hoveredProduct && hoveredProduct.id === pet.id && (
                                            <div className="hover-detail">
                                                <img src={"http://localhost:8080/image/" + hoveredProduct.image} alt={hoveredProduct.name} />
                                                <h3>{hoveredProduct.name}</h3>
                                                <p>Giá: {hoveredProduct.price.toLocaleString()}đ</p>
                                                <p>Giống chó: {hoveredProduct.petDetail.breed}</p>
                                                <p>Phổ Biến: {hoveredProduct.petDetail.popular ? "Rất Phổ Biến" : "Không Phổ Biến"}</p>
                                            </div>
                                        )}
                                    </div>
                                ))
                                    ...
                                    >
                                        ...
                                        {hoveredProduct && hoveredProduct.id === pet.id && (
                                            <div className="hover-detail">
                                                ...
                                                <h3>{hoveredProduct.name}</h3>
                                                <p>Giá: {hoveredProduct.price.toLocaleString()}đ</p>
                                                <p>Giống mèo: {hoveredProduct.petDetail.breed}</p>
                                                <p>Phổ Biến: {hoveredProduct.petDetail.popular ? "Rất Phổ Biến" : "Không Phổ Biến"}</p>
                                            </div>
                                        )}
                                    </div>

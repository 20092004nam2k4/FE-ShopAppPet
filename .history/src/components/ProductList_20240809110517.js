                ...
                <div className="div-tong">
                    <div className="sidebar">
                        ...
                        <h2 className="f aic parent-cat__title">
                            <span className="title">Chó Cảnh</span>
                        </h2>
                        <div className="tabs-wrapper">
                            <div className="arrow-btns">
                                <button onClick={handlePrevDogs}>{'<'}</button>
                                <button onClick={handleNextDogs}>{'>'}</button>
                            </div>
                            <div className="tabs">
                                {TabDogs.slice(startIndexDogs, startIndexDogs + 5).map(tab => (
                                    <div
                                        key={tab}
                                        className={`tab ${tab === activeTabDogs ? 'active' : ''}`}
                            ...
                            <div className="product-grid">
                                {dogs.map((dog) => (
                                    <div
                                        key={dog.id}
                                        className="product"
                                        onClick={() => handleProductClick(dog.id)}
                                    >
                                        <img src={`http://localhost:8080/images/${dog.imageUrl}`} alt={dog.name} />
                                        <h3>{dog.name}</h3>
                                        <p>{dog.price} đ</p>
                                    </div>
                                ))}
            ...
            <div className="product-list container">
                <div className="div-tong">
                    <div className="sidebar">
                        ...
                        <h2 className="f aic parent-cat__title">
                            <span className="title">Mèo Cảnh</span>
                        </h2>
                        <div className="tabs-wrapper">
                            <div className="arrow-btns">
                                <button onClick={handlePrevCats}>{'<'}</button>
                                <button onClick={handleNextCats}>{'>'}</button>
                            </div>
                            <div className="tabs">
                                {TabCats.slice(startIndexCats, startIndexCats + 5).map(tab => (
                                    <div
                                        key={tab}
                                        className={`tab ${tab === activeTabCats ? 'active' : ''}`}
                        ...
                        {loadingCats ? (
                            ...
                            <div className="product-grid">
                                {cats.map((cat) => (
                                    ...
                                    >
                                        <img src={`http://localhost:8080/images/${cat.imageUrl}`} alt={cat.name} />
                                        <h3>{cat.name}</h3>
                                        <p>{cat.price} đ</p>
                                    </div>
                                ))}

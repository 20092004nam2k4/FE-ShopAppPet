...
const ProductList = () => {
    ...
    useEffect(() => {
        const category = TabDogs.includes(activeTab) ? 'Chó' : 'Mèo';
        axios.get('http://localhost:8080/api/pets', { params: { category, breed: activeTab, page: 0, size: 4 } })
            .then(response => {
    ...
    const handleNext = () => {
        const tabs = TabDogs.includes(activeTab) ? TabDogs : TabCats;

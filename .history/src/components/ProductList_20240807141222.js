...
const ProductList = () => {
    ...
    const [activeCategory, setActiveCategory] = useState('Chó');
    ...
    // Hàm fetch dữ liệu pets
    const fetchPets = (category, breed) => {
        setLoadingDogs(category === 'Chó');
        setLoadingCats(category === 'Mèo');
        ...
        axios.get('http://localhost:8080/api/pets', {
            ...
            .catch(error => {
                console.error(`There was an error fetching the ${category.toLowerCase()}s!`, error);
                if (category === 'Chó') setLoadingDogs(false);
                else setLoadingCats(false);
            });
    };

    // Gọi fetch dữ liệu khi activeCategory hoặc activeTab thay đổi
    useEffect(() => {
        fetchPets(activeCategory, activeTab === 'Phổ Biến nhất' ? 'popular' : activeTab);
    }, [activeCategory, activeTab]);

    // Hàm xử lý khi nhấn nút tiếp theo
    const handleNext = () => {
        if (startIndex + 5 < (activeCategory === 'Chó' ? TabDogs.length : TabCats.length)) {

...
const ProductList = () => {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:8080/api/pets') // Cập nhật địa chỉ API nếu cần
            .then(response => {
                // Phân loại dữ liệu thành chó và mèo
                const allPets = response.data.content;
                const dogsList = allPets.filter(Pet => Pet.Category === 'Chó');
                const catsList = allPets.filter(Pet => Pet.Category === 'Mèo');

                setDogs(dogsList); // Lưu danh sách chó vào state
                setCats(catsList); // Lưu danh sách mèo vào state

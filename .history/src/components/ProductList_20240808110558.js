...
const DogSection = ({ activeTab, startIndex, handleTabChange, handleNext, handlePrev, dogs, loadingDogs, onProductClick }) => (
...
const CatSection = ({ activeTab, startIndex, handleTabChange, handleNext, handlePrev, cats, loadingCats, onProductClick }) => (
    <div className="product-list container">
        <div className="div-tong">
            <div className="sidebar">
                <div className="sidebar__top-image">
...
const ProductList = () => {
    ...
    useEffect(() => {
        const fetchPets = async () => {
            try {
                const breed = TabDogs.includes(activeTab) ? getBreedForDogs(activeTab) : getBreedForCats(activeTab); // Determine breed based on tab

                const fetchDogs = axios.get('http://localhost:8080/api/pets', {
                    params: {
                        category: 'Chó',
                        breed: breed,
                        popular: TabDogs.includes(activeTab) && activeTab === 'Phổ Biến nhất',
                        page: 0,
                        size: 4
                    }
                });
                const fetchCats = axios.get('http://localhost:8080/api/pets', {
                    params: {
                        category: 'Mèo',
                        breed: breed,
                        popular: TabCats.includes(activeTab) && activeTab === 'Phổ Biến nhất',
                        page: 0,
                        size: 4
                    }
                });
